...
# 2. Hands control to interactive_menu.py, which:
...
#   Those names live in gc_scraper.py's DIVISIONS dict. Python can import
#   that dict directly — no duplication. Bash cannot. Keeping one source
#   of truth (DRY principle) means adding a team in gc_scraper.py
#   automatically updates the menu with zero extra work.
# ─────────────────────────────────────────────────────────────────────────────

# Navigate to Scripts/ so all python3 calls find gc_scraper.py etc.
