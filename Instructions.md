...
|---|---|---|---|
| `Scout_Development/Scripts/gen_reports.py` | 2067 | Scout | Stat engine + PDF generator (hitting) |
...
|   |   |-- gen_reports.py           <- Step 3: stat engine + hitting PDFs
...
| 3 | `gen_reports.py` | Parses game files, computes batting stats + archetypes, generates hitting PDFs |
...
python3 gen_reports.py --division Majors             # Step 3
...
### gen_reports.py (Step 3 -- Hitting stat engine + PDF generator)
...
| `OUTCOME_TYPES` | ~35 | Outcome string -> code mapping (must sync with gen_reports.py) |
...
**Step 4 integration:** `run_pipeline()` calls `gen_pitching.py` from `Pitching_Savant/Scripts/` after gen_reports.py. Path resolved via `SPRING_DIR / "Pitching_Savant" / "Scripts" / "gen_pitching.py"`.
...
**Imports from Scout Development:** `DIVISIONS` dict from `scrape_gc_playbyplay.py` (team IDs, slugs) and `DIVISIONS` dict from `gen_reports.py` (folder paths). Uses shared venv.

| Function | ~Line | Purpose |
|---|---|---|
| `setup_logging()` | ~103 | Configures file + console logging |
| **Parsing** | | |
| `parse_outcome()` | ~189 | Maps play description -> outcome code (mirrors gen_reports.py) |
...
**Fix:** `scrape_gc_boxscores.py` promotes both to 5-char keys (`Bri A`, `Ben A`) and writes `_collision_map`. `gen_reports.py._disambiguate_pas()` alternates PA assignments by batting order.
